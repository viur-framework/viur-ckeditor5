...
function viewToModelImageAttributeConverter( dataFilter: DataFilter ) {
	return ( dispatcher: UpcastDispatcher ) => {
		dispatcher.on( 'element:img', ( evt, data, conversionApi ) => {
			...
			preserveElementAttributes( viewImageElement, getHtmlAttributeName( 'img' ) );
...
function modelToViewImageAttributeConverter() {
	return ( dispatcher: DowncastDispatcher ) => {
		addInlineAttributeConversion( getHtmlAttributeName( 'img' ) );

		addBlockAttributeConversion( 'img', getHtmlAttributeName( 'img' ) );
